...
class ResnetSetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        self.encoder_seg_2 = self.get_encoder(1, n_downsampling, ngf, norm_layer, use_dropout, n_blocks, padding_type, use_bias)
        ...
        self.decoder_seg_2 = self.get_decoder(1, n_downsampling, 3 * ngf, norm_layer, use_bias)  # 3*ngf
    ...
    def forward(self, inp):
        ...
        #ADD_ON
        segs = inp[:, self.input_nc:self.input_nc+2, :, :]  # (B, CA, W, H)
        segs_2 = inp[:, self.input_nc+2:, :, :]  # (B, CA, W, H)
        ...
        #ADD_NEW
        mean_2 = (segs_2 + 1).mean(0).mean(-1).mean(-1)
        if mean_2.sum() == 0:
            mean_2[0] = 1  # forward at least one segmentation
        ...
        #ADD_NEW
        enc_segs_2 = list()
        for i in range(segs_2.size(1)):
            if mean_2[i] > 0:  # skip empty segmentation
                seg_2 = segs_2[:, i, :, :].unsqueeze(1)
                enc_segs_2.append(self.encoder_seg_2(seg_2))
        enc_segs_2 = torch.cat(enc_segs_2)
        enc_segs_2_sum = torch.sum(enc_segs_2, dim=0, keepdim=True)  # aggregated set_2 feature

        # run decoder
        #ADD_NEW
        '''
        # sum_segs = torch.add(enc_segs_sum,enc_segs_2_sum)
        sum_segs = torch.max(enc_segs_sum,enc_segs_2_sum)
        feat = torch.cat([enc_img, sum_segs], dim=1)
        # feat = torch.cat([enc_img, enc_segs_sum,enc_segs_2_sum], dim=1)
        ...
        for i in range(segs_2.size(1)):
            if mean_2[i] > 0:
                enc_seg_2 = enc_segs_2[idx].unsqueeze(0)  # (1, ngf, w, h)
                idx += 1  # move to next index
                feat = torch.cat([enc_seg_2, enc_img, enc_segs_2_sum], dim=1)
                dec_seg_2 = self.decoder_seg_2(feat)
                out +=[dec_seg_2]
                enc_dec_segs_2.append(self.encoder_seg_2(dec_seg_2))
            else:
                out += [segs_2[:, i, :, :].unsqueeze(1)]  # skip empty segmentation
...
class NLayerSetDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False):
        ...
        self.feature_seg_2 = self.get_feature_extractor(1, ndf, n_layers, kw, padw, norm_layer, use_bias)
        self.classifier = self.get_classifier(2 * ndf, n_layers, kw, padw, norm_layer, use_sigmoid)  # 2*ndf
        self.classifier_2 = self.get_classifier(2 * ndf, n_layers, kw, padw, norm_layer, use_sigmoid)  # 2*ndf
    ...
    def forward(self, inp):
        ...
        img = inp[:, :self.input_nc, :, :]  # (B, CX, W, H)
        #ADD_NEW
        segs = inp[:, self.input_nc:self.input_nc+4, :, :]  # (B, CA, W, H)
        segs_2 = inp[:, self.input_nc+4:, :, :]  # (B, CA, W, H)
        ...
        for i in range(segs_2.size(1)):
            if mean[i] > 0:  # skip empty segmentation
                seg_2 = segs_2[:, i, :, :].unsqueeze(1)
                feat_segs_2.append(self.feature_seg_2(seg_2))
        feat_segs_2_sum = torch.sum(torch.stack(feat_segs_2), dim=0)  # aggregated set feature
