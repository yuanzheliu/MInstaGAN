...
class InstaGANModel(BaseModel):
	...
	def initialize(self, opt):
		...
		#ADD_NEW
		visual_names_A_seg_2 = ['real_A_seg_2', 'fake_B_seg_2', 'rec_A_seg_2']
		visual_names_B_seg_2 = ['real_B_seg_2', 'fake_A_seg_2', 'rec_B_seg_2']
		#ADD_CHANGE
		self.visual_names = visual_names_A_img + visual_names_A_seg + visual_names_A_img+ visual_names_A_seg_2+visual_names_B_img + visual_names_B_seg +visual_names_B_img+visual_names_B_seg_2
	...
	def set_input(self, input):
		...
		#ADD_NEW
		real_A_segs_2 = input['A_segs_2' if AtoB else 'B_segs_2']
		real_B_segs_2 = input['B_segs_2' if AtoB else 'A_segs_2']
		self.real_A_segs_2 = self.select_masks(real_A_segs_2).to(self.device)
		self.real_B_segs_2 = self.select_masks(real_B_segs_2).to(self.device)

		#ADD_ON
		self.real_A = torch.cat([self.real_A_img, self.real_A_segs,self.real_A_segs_2], dim=1)
		self.real_B = torch.cat([self.real_B_img, self.real_B_segs,self.real_B_segs_2], dim=1)
		...
		self.real_A_seg = self.merge_masks(self.real_A_segs)  # merged mask
		self.real_B_seg = self.merge_masks(self.real_B_segs)  # merged mask
		self.real_A_seg_2 = self.merge_masks(self.real_A_segs_2)  # merged mask
		self.real_B_seg_2 = self.merge_masks(self.real_B_segs_2)  # merged mask
	...
	def forward(self, idx=0):
		...
		#ADD_NEW
		self.real_A_seg_2_sng = self.real_A_segs_2[:, N*idx:N*(idx+1), :, :]  # ith mask
		self.real_B_seg_2_sng = self.real_B_segs_2[:, N*idx:N*(idx+1), :, :]  # ith mask
		empty_2 = -torch.ones(self.real_A_seg_2_sng.size()).to(self.device)  # empty image
		...
		if self.forward_A:
    		#ADD_ON
			self.real_A_sng = torch.cat([self.real_A_img_sng, self.real_A_seg_sng,self.real_A_seg_2_sng], dim=1)
			...
			#ADD_ON
			self.fake_B_img_sng, self.fake_B_seg_sng,self.fake_B_seg_2_sng = self.split(self.fake_B_sng)
			self.rec_A_img_sng, self.rec_A_seg_sng,self.rec_A_seg_2_sng = self.split(self.rec_A_sng)
			...
			#ADD_NEW
			fake_B_seg_2_list = self.fake_B_seg_2_list + [self.fake_B_seg_2_sng]  # not detach
			for i in range(self.ins_iter - idx - 1):
				fake_B_seg_2_list.append(empty_2)
			self.fake_B_seg_2_mul = torch.cat(fake_B_seg_2_list, dim=1)

			#ADD NEW
			# fake_B_seg_together = torch.div(torch.add(self.fake_B_seg_mul,self.fake_B_seg_2_mul),2)
			fake_B_seg_together = torch.max(self.fake_B_seg_mul,self.fake_B_seg_2_mul)
			# fake_B_seg_together = torch.add(self.fake_B_seg_mul,self.fake_B_seg_2_mul)
			# fake_B_seg_together[fake_B_seg_together == -2.0] = -1.0

			#ADD_ON
			# self.fake_B_mul = torch.cat([self.fake_B_img_sng, fake_B_seg_together], dim=1)
			# self.fake_B_mul = torch.cat([self.fake_B_img_sng, self.fake_B_seg_mul], dim=1)
			self.fake_B_mul = torch.cat([self.fake_B_img_sng, self.fake_B_seg_mul,self.fake_B_seg_2_mul], dim=1)

		# forward B
		if self.forward_B:
    		#ADD_ON 
			self.real_B_sng = torch.cat([self.real_B_img_sng, self.real_B_seg_sng,self.real_B_seg_2_sng], dim=1)
			...
			#ADD_ON 
			self.fake_A_img_sng, self.fake_A_seg_sng,self.fake_A_seg_2_sng = self.split(self.fake_A_sng)
			self.rec_B_img_sng, self.rec_B_seg_sng, self.rec_B_seg_2_sng = self.split(self.rec_B_sng)
			...
			#ADD_NEW
			fake_A_seg_2_list = self.fake_A_seg_2_list + [self.fake_A_seg_2_sng]  # not detach
			for i in range(self.ins_iter - idx - 1):
				fake_A_seg_2_list.append(empty_2)
			self.fake_A_seg_2_mul = torch.cat(fake_A_seg_2_list, dim=1)
			
			#ADD_NEW
			# fake_A_seg_together = torch.div(torch.add(self.fake_A_seg_mul,self.fake_A_seg_2_mul),2)
			fake_A_seg_together = torch.max(self.fake_A_seg_mul,self.fake_A_seg_2_mul)

			#ADD_NEW
			# self.fake_A_mul = torch.cat([self.fake_A_img_sng,fake_A_seg_together], dim=1)
			# self.fake_A_mul = torch.cat([self.fake_A_img_sng, self.fake_A_seg_mul], dim=1)
			self.fake_A_mul = torch.cat([self.fake_A_img_sng, self.fake_A_seg_mul,self.fake_A_seg_2_mul], dim=1)
	...
	def optimize_parameters(self):
		...
		#ADD_NEW 
		self.fake_A_seg_2_list = list()
		self.fake_B_seg_2_list = list()
		self.rec_A_seg_2_list = list()
		self.rec_B_seg_2_list = list()
		...
		for i in range(self.ins_iter):
			...
			#ADD_NEW
			self.fake_A_seg_2_list.append(self.fake_A_seg_2_sng.detach())
			self.fake_B_seg_2_list.append(self.fake_B_seg_2_sng.detach())
			self.rec_A_seg_2_list.append(self.rec_A_seg_2_sng.detach())
			self.rec_B_seg_2_list.append(self.rec_B_seg_2_sng.detach())
			...
			if i == self.ins_iter - 1:  # last
				...
				#ADD_NEW
				self.fake_A_seg_2 = self.merge_masks(self.fake_A_seg_2_mul)
				self.fake_B_seg_2 = self.merge_masks(self.fake_B_seg_2_mul)
				self.rec_A_seg_2 = self.merge_masks(torch.cat(self.rec_A_seg_2_list, dim=1))
				self.rec_B_seg_2 = self.merge_masks(torch.cat(self.rec_B_seg_2_list, dim=1))
